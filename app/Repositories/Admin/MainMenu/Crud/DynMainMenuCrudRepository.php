...
namespace App\Repositories\Admin\MainMenu\Crud;
...
class  DynMainMenuCrudRepository extends BaseRepository implements IDynMainMenuCrudRepository {
    ...
    {
       return $this->getPageDefault(model: $this->DynMainMenu, id: $id);
    }
    ...
    public function store($request) : JsonResponse
    {
        ...
        try {
            ...
            DB::rollback();
            $this->saveError($this->getSystemError(['name' => 'DynMainMenu_store_error']), $e);
    ...
    public function update($request,$id) : JsonResponse
    {
        ...
        if($row->isDirty()){
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: " DynMainMenu ".$row?->name.' was updated by '.$request?->auth?->name,request: $request, row: $rowRef, type: 'to'));
                DB::commit();
                return $this->response(['type' => 'success','data' => $data]);
            } catch (\Exception $e) {
                DB::rollback();
                $this->saveError($this->getSystemError(['name'=>'DynMainMenu_update_error']), $e);
        ...
        if (count($i) > 0) {
            ...
            if (count($dirty) > 0) {
                ...
                try {
                    ...
                    $this->saveTractAction($this->getTrackData(title: "DynMainMenu list was updated by ".$request?->auth?->name, request: $request));
                    DB::commit();
                    return $this->response(['type' => 'success','data' => $data]);
                } catch (\Exception $e) {
                    DB::rollback();
                    $this->saveError($this->getSystemError(['name' => 'DynMainMenu_bulk_update_error']), $e);
                    return $this->response(['type' => 'wrong', 'lang' => 'server_wrong']);
                }
            } else {
                return $this->response(['type' => 'noUpdate', 'title' =>  '<span class="text-success"> '.pxLang($request->lang,'','common.no_change').'  </span>']);
    ...
    {
        $errors = [];
        $i = DynMainMenu::whereIn('id',$request->ids)->select(['id'])->get();
        if (count($i) > 0) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: "DynMainMenu list was deleted by ".$request?->auth?->name, request: $request));
                DB::commit();
                return $this->response(['type' => 'success',"data"=>$data]);
            } catch (\Exception $e) {
                DB::rollback();
